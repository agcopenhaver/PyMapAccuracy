...
def stehman2014(
    ...
    if not isinstance(Nh_strata, dict):
        raise ValueError("Nh_strata must be a dictionary with stratum labels as keys and areas as values.")
    if not all(isinstance(k, (str, int, float)) for k in Nh_strata.keys()):
         raise ValueError("Nh_strata keys must be valid stratum labels (str, int, or float).")
    if not all(isinstance(v, (int, float)) and v >= 0 for v in Nh_strata.values()):
        raise ValueError("Nh_strata values must be non-negative numbers representing stratum areas.")
    ...
    if len(set(lengths)) > 1:
        raise ValueError(f"Input vectors must have the same length. Got: s={len(s)}, r={len(r)}, m={len(m)} elements. "
    ...
    if not set(unique_s).issubset(set(strata_keys)):
        ...
        raise ValueError(f"Stratum labels in 's' not found in Nh_strata keys: {missing_strata}. "
    ...
    if unused_strata:
        warnings.warn(f"Strata in Nh_strata not found in sample data 's': {'; '.join(map(str, sorted(unused_strata)))}. "
    ...
    strata_levels = list(Nh_strata_filt.keys())  # Preserve stratum ordering for consistent indexing
    ...
    if N == 0:
        raise ValueError("Total population size (sum of Nh_strata values) cannot be zero. "
    ...
    unique_rm = sorted(list(set(r.unique()) | set(m.unique())))
    if order is None:
        ...
        if missing_in_order:
            raise ValueError(f"Argument 'order' must include all class labels found in reference and map data. "
                           f"Missing: {'; '.join(map(str, sorted(list(missing_in_order))))}. "
    ...
    def eq2(yu_col: str) -> float:
        ...
        """
        return (sample[yu_col] / sample['incl']).sum() / N

    def eq25(Nh_dict: Dict[Any, float], nh_series: pd.Series, var_series: pd.Series) -> float:
        """
        ...
        for h in strata_levels: # Iterate through strata names
            ...
            var_h = var_series.get(h, 0) # Get variance for stratum h, default 0 if missing
            if nh > 0: # Avoid division by zero
                 # Finite population correction (1 - nh/Nh)
                 fpc = (1 - nh / Nh) if Nh > 0 else 1 # If Nh is 0, stratum shouldn't exist, but handle defensively
                 term += (Nh**2 * fpc * var_h) / nh
        return term / (N**2)

    def eq28(R_ratio, X_est, Nh_dict, nh_series, vary_series, varx_series, cov_series):
    ...
    nh_series = s.value_counts().reindex(strata_levels, fill_value=0) # Counts per stratum, ordered like Nh_strata_filt
    Nh_series = pd.Series(Nh_strata_filt).reindex(strata_levels, fill_value=0) # Population per stratum, ordered

    if (nh_series < 2).any():
        single_obs_strata = nh_series[nh_series < 2].index.tolist()
        warnings.warn(f"The following strata include only one observation: {'; '.join(map(str, single_obs_strata))}. "
                     f"Variance estimates for these strata will be zero, which may affect standard error calculations. "
                     f"Consider increasing sample size in these strata if possible.", stacklevel=2)
    ...
    if problematic_incl.any():
        ...
        raise ValueError(f"Invalid inclusion probabilities detected for samples in strata: {list(problematic_strata)}. "
    ...
    yu_m_r_cols = {} # Store column names for matrix
    for i, map_class in enumerate(map_classes):
        yu_m_r_cols[map_class] = {}
        for j, ref_class in enumerate(ref_classes):
            col_name = f'yu_m{i}r{j}'
            sample[col_name] = ((sample['m'] == map_class) & (sample['r'] == ref_class)).astype(float)
    ...
    grouped = sample.groupby('s', observed=False) # observed=False includes strata with 0 samples if they were in levels
    tmean = grouped.mean(numeric_only=True)
    tvar = grouped.var(numeric_only=True, ddof=1).fillna(0) # Replace NaN variance (e.g., single sample strata) with 0
    ...
    Va_list = [eq25(Nh_strata_filt, nh_series, tvar[f'yu_r{j}']) for j in range(n_ref_classes)]
    SEa = pd.Series(np.sqrt(np.maximum(Va_list, 0)), index=ref_classes, name="SE_Area") # Use maximum to avoid sqrt(<0)
    ...
    for i, map_class in enumerate(map_classes):
        yu_col = f'yu_U{i}'
        xu_col = f'xu_U{i}'
        sample[yu_col] = ((sample['m'] == map_class) & (sample['m'] == sample['r'])).astype(float)
    ...
    for j, ref_class in enumerate(ref_classes):
        yu_col = f'yu_P{j}'
        xu_col = f'xu_P{j}'
        sample[yu_col] = ((sample['r'] == ref_class) & (sample['r'] == sample['m'])).astype(float)
    ...
    tvar = grouped.var(numeric_only=True, ddof=1).fillna(0).reindex(strata_levels, fill_value=0)
    ...
    for h, group_df in sample.groupby('s', observed=False):
         ...
         for i in range(n_map_classes):
             ...
             if yu_U_cols[i] in group_df and xu_U_cols[i] in group_df:
                 ...
                 cov_U_dict[h][i] = 0.0 # Should not happen if columns were added correctly
    ...
    cov_U_series_list = []
    for i in range(n_map_classes):
        cov_series = pd.Series({h: cov_U_dict.get(h, {}).get(i, 0.0) for h in strata_levels}, name=f'cov_U{i}')
        cov_U_series_list.append(cov_series)

    cov_P_series_list = []
    for j in range(n_ref_classes):
        cov_series = pd.Series({h: cov_P_dict.get(h, {}).get(j, 0.0) for h in strata_levels}, name=f'cov_P{j}')
    ...
    for i in range(n_map_classes):
        ...
        if pd.isna(R_ratio) or X_est == 0: # If UA is NaN or denominator is 0, SE is NaN
             Vua_list.append(np.nan)
             continue
        vary_series = tvar[yu_U_cols[i]]
        varx_series = tvar[xu_U_cols[i]]
        cov_series = cov_U_series_list[i]
        Vua = eq28(R_ratio, X_est, Nh_strata_filt, nh_series, vary_series, varx_series, cov_series)
        Vua_list.append(Vua)
    SEua = pd.Series(np.sqrt(np.maximum(Vua_list, 0)), index=map_classes, name="SE_UA") # Use maximum to avoid sqrt(<0)
    ...
    for j in range(n_ref_classes):
        ...
        if pd.isna(R_ratio) or X_est == 0: # If PA is NaN or denominator is 0, SE is NaN
            Vpa_list.append(np.nan)
            continue
        vary_series = tvar[yu_P_cols[j]]
        varx_series = tvar[xu_P_cols[j]]
        cov_series = cov_P_series_list[j]
        Vpa = eq28(R_ratio, X_est, Nh_strata_filt, nh_series, vary_series, varx_series, cov_series)
        Vpa_list.append(Vpa)
    SEpa = pd.Series(np.sqrt(np.maximum(Vpa_list, 0)), index=ref_classes, name="SE_PA") # Use maximum to avoid sqrt(<0)
...
def olofsson(
    ...
    if not isinstance(Nh, dict):
        raise ValueError("Nh must be a dictionary with map class labels as keys and areas as values.")
    if not all(isinstance(k, (str, int, float)) for k in Nh.keys()):
         raise ValueError("Nh keys (map class labels) must be valid labels (str, int, or float).")
    ...
    if len(Nh) != len(set(Nh.keys())):
         raise ValueError("Repeated map class names detected in Nh. Each map class must appear only once.")
    ...
    lengths = [len(r), len(m)]
    if len(set(lengths)) > 1:
        raise ValueError(f"Input vectors must have the same length. Got: r={len(r)}, m={len(m)} elements. "
    ...
    if missing_map_strata:
        raise ValueError(f"Map class labels in sample data 'm' not found in Nh keys: {missing_map_strata}. "
    ...
    if extra_ref_classes:
        warnings.warn(f"Reference classes not found in map strata (Nh): {extra_ref_classes}. "
    ...
    if extra_map_classes:
        warnings.warn(f"Map classes not found in reference data: {extra_map_classes}. "
    ...
    if unused_nh_classes:
        warnings.warn(f"Map strata in Nh not found in sample data 'm': {unused_nh_classes}. "
    ...
    # Sampling settings
    q = len(class_order) # Total number of unique classes (map + reference)
    q_map = len(map_classes_nh) # Number of map strata
    ...
    if (nh < 2).any():
        ...
        warnings.warn(f"The following strata include only one observation: {'; '.join(map(str, single_obs_strata))}. Variance estimates for these strata might be zero or unstable.", stacklevel=2)
    ...
    diag_props = np.diag(props.reindex(index=class_order, columns=class_order, fill_value=0)) # Ensure square matrix for diag


    OA = diag_props.sum() # Overall accuracy is a single value

    # User's Accuracy (UA) - indexed by map class
    map_totals_est = props.sum(axis=1).reindex(map_classes_nh, fill_value=0) # Estimated proportion for each map class
    ua_diag = np.diag(props.reindex(index=map_classes_nh, columns=map_classes_nh, fill_value=0)) # Diagonal elements corresponding to map classes

    with np.errstate(divide='ignore', invalid='ignore'):
        UA = pd.Series(ua_diag / map_totals_est, index=map_classes_nh, name="UA", dtype=float)
    UA = UA.fillna(0.0) # If map_totals_est is 0, UA is 0 or NaN -> 0

    # Producer's Accuracy (PA) - indexed by reference class
    ref_totals_est = props.sum(axis=0).reindex(class_order, fill_value=0) # Estimated area proportion for each reference class (Area estimate)
    pa_diag = np.diag(props.reindex(index=class_order, columns=class_order, fill_value=0)) # Diagonal elements corresponding to all classes
    
    # Check which reference classes actually have samples in the reference data
    ref_sample_counts = pd.Series(r_cat).value_counts().reindex(class_order, fill_value=0)
    ...
    nh_safe = nh.replace(1, np.inf).replace(0, np.inf) # Avoid division by zero or negative variance for nh=1
    ...
    term_var_a = (Wh**2).values[:, np.newaxis] * (p_ij_stratum * (1 - p_ij_stratum)).div(nh_safe - 1, axis=0)
    VAR_A = term_var_a.sum(axis=0).reindex(class_order, fill_value=0) # Sum over strata (i) for each ref class (j)
    ...
    for j_idx, j_class in enumerate(class_order):
        ...
        for i_idx, i_class in enumerate(map_classes_nh): # Iterate through map strata i
            nh_i = nh.get(i_class, 0)
            if nh_i < 2: # Skip strata with < 2 samples for variance calc
                continue

            wh_i = Wh.get(i_class, 0)
            p_ij_strat = p_ij_stratum.loc[i_class, j_class] # Proportion p_ij within stratum i
    ...
    output_matrix = output_matrix.reindex(index=class_order, columns=class_order, fill_value=np.nan)
    ...
    matrix_aligned = matrix.reindex(index=class_order, columns=class_order, fill_value=0)
    output_matrix[matrix_aligned == 0] = np.nan

    if margins:
        # Calculate sums, replacing NaN with 0 for summation before adding margins
        row_sum = output_matrix.sum(axis=1, skipna=True).reindex(class_order, fill_value=0)
        col_sum = output_matrix.sum(axis=0, skipna=True).reindex(class_order, fill_value=0)
        ...
        output_matrix, col_sum_df = output_matrix.align(col_sum_df, axis=1, join='outer', fill_value=np.nan)
        output_matrix = pd.concat([output_matrix, col_sum_df])
        # Reorder rows/cols potentially? R code adds sum last.
        final_row_order = class_order + ['sum']  # Use class_order instead of map_classes_nh
        final_col_order = class_order + ['sum']
        output_matrix = output_matrix.reindex(index=final_row_order, columns=final_col_order)


    return {
        'OA': OA,
        'UA': UA.reindex(class_order).fillna(np.nan), # Show UA for all classes, NaN if not a map class
        'PA': PA,
        'area': Area,
        'SEoa': SEoa,
