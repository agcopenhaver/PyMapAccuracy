...
def test_stehman2014_valid_input():
    ...
    s = ['elevation_low', 'elevation_low', 'elevation_med', 'elevation_med', 'elevation_high', 'elevation_high']  # Elevation strata
    r = ['forest', 'grassland', 'grassland', 'water', 'water', 'water']                                          # Reference classes  
    m = ['forest', 'forest', 'grassland', 'grassland', 'water', 'water']                                         # Map classes (different from strata)
    Nh_strata = {'elevation_low': 1000, 'elevation_med': 800, 'elevation_high': 600}                             # Areas by elevation stratum
...
def test_stehman2014_strata_equal_map_classes():
    ...
    m = ['forest', 'forest', 'water', 'water', 'urban', 'urban']    # Map classes (same as strata)
    Nh_strata = {'forest': 1000, 'water': 800, 'urban': 600}        # Areas by stratum (which = map class)
...
def test_stehman2014_different_lengths_handled_gracefully():
    ...
    Nh_strata = {'stratum_1': 100, 'stratum_2': 200}  # Areas by stratum, not by map class
...
def test_stehman2014_invalid_Nh_strata():
    ...
    s = ['geographic_north', 'geographic_north', 'geographic_south', 'geographic_south']  # Geographic strata
    r = ['forest', 'water', 'water', 'grassland']                                        # Reference classes
    m = ['forest', 'forest', 'water', 'water']                                           # Map classes (different from strata)
    
    # Test negative area
    Nh_strata_negative = {'geographic_north': 1000, 'geographic_south': -500}  # Invalid negative area
    with pytest.raises(ValueError, match="Nh_strata values must be non-negative numbers representing stratum areas"):
        stehman2014(s, r, m, Nh_strata_negative)
        
    # Test non-dictionary
    Nh_strata_list = [1000, 500]  # Should be dict, not list
    with pytest.raises(ValueError, match="Nh_strata must be a dictionary with stratum labels as keys and areas as values"):
...
def test_stehman2014_missing_strata():
    ...
    Nh_strata = {'admin_zone_A': 500, 'admin_zone_B': 300}         # Missing 'admin_zone_missing'
    
    with pytest.raises(ValueError, match="Stratum labels in 's' not found in Nh_strata keys: .*admin_zone_missing.*All strata in sample data must have corresponding area values"):
        stehman2014(s, r, m, Nh_strata)
...
def test_stehman2014_correctness_perfect():
    ...
    r_perf = ['forest', 'forest', 'water', 'water']           # Reference classes (perfect agreement)
    m_perf = ['forest', 'forest', 'water', 'water']           # Map classes (perfect agreement with reference)
    Nh_perf = {'admin_A': 100, 'admin_B': 100}               # Areas by administrative stratum (not by land cover class)
    ...
    expected_matrix = pd.DataFrame(expected_matrix_data, index=['forest', 'water', 'sum'])
...
def test_stehman2014_paper_example():
    ...
    m = ['A'] * 7 + ['B'] * 3 + ['A'] * 1 + ['B'] * 11 + ['C'] * 6 + ['B'] * 2 + ['D'] * 10
    r = (['A'] * 5 + ['C'] * 1 + ['B'] * 1 + ['A'] * 1 + ['B'] * 1 + ['C'] * 1 +  # Stratum A (10)
         ['A'] * 1 + ['B'] * 5 + ['A'] * 2 + ['B'] * 2 +                         # Stratum B (10)
         ['C'] * 5 + ['D'] * 2 + ['B'] * 2 + ['A'] * 1 +                         # Stratum C (10)
         ['D'] * 7 + ['C'] * 2 + ['B'] * 1)                                      # Stratum D (10)
    ...
    result = stehman2014(s, r, m, Nh_strata, order=order, margins=False)  # Match R example default
...
def test_stehman2014_invalid_order():
    ...
    with pytest.raises(ValueError, match=r"Argument 'order' must include all class labels found in reference and map data.*Missing.*2"):
...
def test_stehman2014_extreme_class_imbalance():
    ...
    assert result['area']['building'] == pytest.approx(0.95, abs=0.1)  # Approximately 95%
...
def test_stehman2014_empty_strata():
    ...
    Nh_strata = {'coast': 100, 'mountain': 100, 'desert': 200}  # 'desert' has area but no samples
    
    # Should handle gracefully with warning about unused stratum
    with pytest.warns(UserWarning, match="Strata in Nh_strata not found in sample data"):
        result = stehman2014(s, r, m, Nh_strata)
...
def test_stehman2014_single_sample_stratum():
    ...
    s = ['high_elev', 'low_elev', 'low_elev', 'low_elev']   # Unequal stratum sample sizes
...
def test_stehman2014_extra_map_class():
    ...
    assert list(result['matrix'].columns[:-1]) == expected_classes  # Exclude 'sum' column
...
def test_stehman2014_extra_ref_class():
    ...
    assert 'grassland' in result['PA'].index  # Should have PA for reference class 'grassland'
    assert 'forest' in result['UA'].index
...
def test_stehman2014_all_classes_in_one_stratum():
    ...
    Nh_strata = {'central': 1000, 'peripheral': 500}       # Two strata, but samples only in one
...
def test_stehman2014_class_appears_in_multiple_strata():
    ...
    r = ['forest', 'forest', 'forest', 'desert', 'forest', 'grassland']  # 'forest' in multiple strata
    m = ['forest', 'forest', 'forest', 'desert', 'forest', 'grassland']  # Perfect classification
...
def test_stehman2014_confidence_intervals_structure():
    ...
    ci_keys = ['CIoa', 'CIua', 'CIpa', 'CIa', 'CI_halfwidth_oa', 'CI_halfwidth_ua', 'CI_halfwidth_pa', 'CI_halfwidth_a']
    for key in ci_keys:
