...
def test_olofsson_example1_2013():
    ...
    # Area
    assert result['area']['1'] == pytest.approx(0.02570326, abs=1e-5) # eq. 9 (proportion)
    assert result['area']['1'] * total_area_pixels == pytest.approx(45112.4, abs=1) # eq. 10 (pixels)
    # SE Area
    ...
    assert ci_lower == pytest.approx(45112.4 - 21072.37, abs=2) # Using paper's CI width / 2
    assert ci_upper == pytest.approx(45112.4 + 21072.37, abs=2) # Using paper's CI width / 2
    ...
    expected_pa = pd.Series([0.4806308, 0.9941887, 0.8969259], index=['1', '2', '3'], name='PA')
    expected_se_ua_ci = pd.Series([0.03360292, 0.02892031, 0.03360292], index=['1', '2', '3'], name='SE_UA') # CI half-width / Z_95
    expected_se_pa_ci = pd.Series([0.22453045, 0.01132522, 0.04120541], index=['1', '2', '3'], name='SE_PA') # CI half-width / Z_95

    assert_series_equal(result['UA'].dropna(), expected_ua, check_exact=False, rtol=1e-5)
    assert_series_equal(result['PA'].dropna(), expected_pa, check_exact=False, rtol=1e-5)
    # Compare SE * Z_95
    assert_series_equal(result['SEua'].dropna() * Z_95, expected_se_ua_ci, check_exact=False, rtol=1e-5) # Allow slightly larger tolerance for SE
    assert_series_equal(result['SEpa'].dropna() * Z_95, expected_se_pa_ci, check_exact=False, rtol=1e-5) # Allow slightly larger tolerance for SE
...
def test_olofsson_example2_2013():
    ...
    expected_ua = pd.Series([0.51417, 0.94428, 0.97297], index=['1', '2', '3'], name='UA')
    expected_pa = pd.Series([0.67535, 0.93072, 0.97664], index=['1', '2', '3'], name='PA')
    expected_se_ua_ci_hw = pd.Series([0.06246, 0.02438, 0.01350], index=['1', '2', '3'], name='SE_UA') # CI half-width
    expected_se_pa_ci_hw = pd.Series([0.30458, 0.02938, 0.00960], index=['1', '2', '3'], name='SE_PA') # CI half-width

    assert_series_equal(result['UA'].dropna(), expected_ua, check_exact=False, rtol=1e-2)
    assert_series_equal(result['PA'].dropna(), expected_pa, check_exact=False, rtol=1e-2)
    assert_series_equal(result['SEua'].dropna() * Z_95, expected_se_ua_ci_hw, check_exact=False, rtol=1e-2)
    assert_series_equal(result['SEpa'].dropna() * Z_95, expected_se_pa_ci_hw, check_exact=False, rtol=1e-2)
...
def test_olofsson_table8_2014():
    ...
    mapping = {1: "Deforestation", 2: "Forest gain", 3: "Stable forest", 4: "Stable non-forest"}
    r = [mapping[x] for x in r_num]
    m = [mapping[x] for x in m_num]

    Nh_counts = {"Deforestation": 200000, "Forest gain": 150000,
                 "Stable forest": 3200000, "Stable non-forest": 6450000}
    pixel_area = 30**2
    ...
    expected_ua = pd.Series([0.8800000, 0.7333333, 0.9272727, 0.9630769921], index=class_order, name='UA')
    expected_pa = pd.Series([0.7486614, 0.8471564, 0.9345089, 0.9616090], index=class_order, name='PA')
    expected_se_ua_ci_hw = pd.Series([0.07403962, 0.10075516, 0.03974464, 0.02053312], index=class_order, name='SE_UA') # CI half-width
    expected_se_pa_ci_hw = pd.Series([0.21330593, 0.25440369, 0.03432379, 0.01836120], index=class_order, name='SE_PA') # CI half-width

    assert result['OA'] == pytest.approx(0.9465119, abs=1e-3)
    assert result['SEoa'] * Z_95 == pytest.approx(0.01848328, abs=1e-5)

    assert_series_equal(result['UA'].reindex(class_order), expected_ua, check_exact=False, rtol=1e-5)
    assert_series_equal(result['PA'].reindex(class_order), expected_pa, check_exact=False, rtol=1e-5)
    assert_series_equal(result['SEua'].reindex(class_order) * Z_95, expected_se_ua_ci_hw, check_exact=False, rtol=1e-5)
    assert_series_equal(result['SEpa'].reindex(class_order) * Z_95, expected_se_pa_ci_hw, check_exact=False, rtol=1e-5)
...
def test_olofsson_extra_map_class():
    ...
    m = ['1'] * 97 + ['2'] * 3 + ['3'] * 2 + \
        ['2'] * 279 + ['3'] * 1 + \
        ['1'] * 3 + ['2'] * 18 + ['3'] * 95 + ['4'] * 2 # 2 samples mapped as '4'
    ...
    assert pd.isna(result['SEpa']['4'])  # NaN because no reference samples for class '4'
...
def test_olofsson_extra_ref_class():
    ...
    m = ['1'] * 97 + ['2'] * 3 + ['3'] * 2 + \
        ['2'] * 279 + ['3'] * 1 + \
        ['1'] * 3 + ['2'] * 18 + ['3'] * 97 # Length matches r
    ...
    assert '4' in result['matrix'].index # Now class '4' should appear as a row (symmetrical matrix)
    ...
    assert result['SEpa']['4'] == 0  # Should be 0 because PA is 0, but reference samples exist
...
def test_olofsson_extra_nh_class():
    ...
    actual_ref_classes = ['1', '2', '3', '4']  # The actual reference classes from the data (now includes '4')
    
    assert result['OA'] == pytest.approx(result_orig['OA'])
    assert_series_equal(result['area'][actual_ref_classes], result_orig['area'])  # Subset result to match
    assert_series_equal(result['PA'][actual_ref_classes], result_orig['PA'])      # Subset result to match
    assert result['SEoa'] == pytest.approx(result_orig['SEoa'])
    assert_series_equal(result['SEa'][actual_ref_classes], result_orig['SEa'])    # Subset result to match
    assert_series_equal(result['SEpa'][actual_ref_classes], result_orig['SEpa'])  # Subset result to match
    ...
    common_indices = ['1', '2', '3', '4', 'sum']  # Include 'sum' row/col if margins=True
    assert_frame_equal(result['matrix'].loc[common_indices, common_indices],
                       result_orig['matrix'])
    
    # Verify that row '9' and column '9' are all NaN/0 in result
    test_cols = ['1', '2', '3', '4', 'sum']
    assert all(pd.isna(result['matrix'].loc['9', test_cols]) | (result['matrix'].loc['9', test_cols] == 0))
    assert all(pd.isna(result['matrix'].loc[test_cols, '9']) | (result['matrix'].loc[test_cols, '9'] == 0))
...
def test_olofsson_invalid_input():
    ...
    Nh = {1: 100, 2: 200}
    with pytest.raises(ValueError, match=r"Map class labels in sample data 'm' not found in Nh keys:.*3"):
        olofsson(r, m, Nh)
...
def test_olofsson_single_class():
    ...
    assert result['UA']['A'] == pytest.approx(1.0)
    assert result['PA']['A'] == pytest.approx(1.0)
    assert result['area']['A'] == pytest.approx(1.0)  # Only class, should be 100% of area
...
def test_olofsson_empty_strata():
    ...
    assert result['UA']['C'] == 0  # Map stratum with no samples → 0 estimated area → UA = 0
    assert result['SEua']['C'] == 0  # No variance for unused stratum
    assert pd.isna(result['PA']['C'])  # No reference samples for class 'C' → PA undefined
